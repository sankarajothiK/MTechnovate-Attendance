...
export default function HomePage() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#fdfbfb] via-[#fff8f8] to-[#fbf2f2] text-slate-900 flex flex-col justify-between relative overflow-hidden">
      ...
      {/* Navigation */}
      <nav className="max-w-7xl mx-auto w-full px-6 sm:px-8 py-5 flex items-center justify-between relative z-10 border-b border-[#ebdcdc]">
        <Link href="/" className="flex items-center gap-3.5 group select-none">
          <div className="w-11 h-11 rounded-xl p-1 bg-white ring-2 ring-[#ebdcdc] group-hover:ring-[#b76e79] shadow-xs transition-all overflow-hidden flex items-center justify-center">
          ...
          <div>
            ...
            <p className="text-[10px] font-semibold tracking-wider text-[#a0636d] uppercase mt-0.5">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/attendance"
            className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-gradient-to-r from-[#b76e79] via-[#c5838d] to-[#9e5762] hover:from-[#a8606b] hover:to-[#8c4651] text-white text-xs font-bold tracking-wide shadow-md shadow-[#b76e79]/20 active:scale-95 transition-all"
          >
          ...
          <Link
            href="/login"
            className="px-4 py-2.5 rounded-xl bg-white hover:bg-[#fff7f7] text-[#8e4a55] border border-[#ebdcdc] hover:border-[#b76e79] text-xs font-bold transition-all shadow-2xs"
          >
            Admin Portal
          </Link>
        </div>
      ...
      <main className="max-w-5xl mx-auto w-full px-6 py-12 sm:py-16 text-center space-y-8 relative z-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-4xl mx-auto pt-6 text-left text-xs">
      ...
      {/* Footer */}
      <footer className="max-w-7xl mx-auto w-full px-6 py-6 border-t border-[#ebdcdc] flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-2 relative z-10">
        <p className="font-semibold text-[#8e4a55]">
          © 2026 M Technovate Solutions. All rights reserved. • Innovate at every step
        </p>
        <p className="font-mono text-[11px] text-slate-400 bg-white px-3 py-1 rounded-full border border-[#ebdcdc]">
          Admin Demo: <span className="text-[#8e4a55] font-bold">admin@mtechno.com</span> / <span className="text-[#8e4a55] font-bold">admin123</span>
        </p>
      </footer>
    </div>
  );
