    ...
    <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-md border-b border-[#ebdcdc] shadow-[0_4px_20px_-4px_rgba(183,110,121,0.06)]">
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10">
        <div className="flex h-18 items-center justify-between gap-8">
          ...
          <Link
            href="/admin"
            className="flex items-center gap-3.5 group focus:outline-none select-none shrink-0"
          >
            <div className="w-11 h-11 rounded-xl p-1 bg-white ring-2 ring-[#ebdcdc] group-hover:ring-[#c5838d] shadow-xs transition-all overflow-hidden flex items-center justify-center">
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-[#ebdcdc] bg-white px-6 pt-3 pb-5 space-y-2 shadow-lg">
          {navLinks.map((link) => {
            const Icon = link.icon;
            const isActive = pathname === link.href;
            return (
          ...
          <div className="pt-3 border-t border-[#ebdcdc] flex flex-col gap-2">
            ...
            <button
              type="button"
              onClick={handleLogout}
              className="flex items-center justify-center gap-2 py-2 text-xs font-bold text-rose-700 bg-rose-50 rounded-xl"
            >
              <LogOut className="w-4 h-4" />
              <span>Sign Out</span>
            </button>
